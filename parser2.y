...
declaracao_variavel: tipo lista_identificadores
                   {
                       ...
                       // Adicionando os identificadores como filhos
                       Nodo* ultimo = novo_no;
...
int main(int argc, char **argv) {
    ...
    // Implementar função para liberar a árvore aqui
...
// Estrutura para os nós da árvore
typedef struct Nodo {
    char* tipo; // Tipo do nó (ex: "declaracao", "condicional", etc.)
    char* nome; // Nome da variável (se aplicável)
    char* tipo_variavel; // Tipo da variável (se aplicável)
    int linha; // Número da linha (se aplicável)
    struct Nodo* filho; // Primeiro filho
    struct Nodo* proximo; // Irmão
} Nodo;
...
comando_condicional:
    TOKEN_SE expressao TOKEN_ENTAO comando
    {
        ...
        novo_no->filho = $2; // Expressão
        novo_no->proximo = $4; // Comando
        // Adicionar novo_no à raiz ou à lista
    }
    | TOKEN_SE expressao TOKEN_ENTAO comando TOKEN_SENAO comando
    {
        Nodo* novo_no = malloc(sizeof(Nodo));
        ...
        novo_no->filho = $2; // Expressão
        novo_no->proximo = $4; // Comando
        novo_no->proximo->proximo = $6; // Comando SENAO
    }
    ;
